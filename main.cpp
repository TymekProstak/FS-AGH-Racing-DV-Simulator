...
int main(int argc, char** argv)
{
    ...
    std::string param_file, cones_file, centerline_file, log_file;
    ...
    if (!nh.getParam("centerline_file", centerline_file))
        throw std::runtime_error("Missing required param: centerline_file");
    ...
    ROS_INFO_STREAM("  centerline = " << centerline_file);
    ...
    // The wall-clock loop follows the same runtime-configured step as physics.
    const double simulation_step_s =
        sim.get_parameters().get("simulation_time_step");
    const double loop_hz = 1.0 / simulation_step_s;
